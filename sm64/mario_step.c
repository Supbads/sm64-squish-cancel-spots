...
u32 mario_update_windy_ground(struct MarioState *m)
{
    ...
    if (floor->type == SURFACE_002C)
    {
        ...
        {
            pushSpeed = 3.2f + (g->globalTimer % 4);
        }
...
static u32 perform_ground_quarter_step(struct MarioState *m, Vec3f nextPos)
{
    ...
    // printf("  init floor height = %f\n", m->floorHeight);
    // printf("  new floor height = %f\n", floorHeight);
    // printf("  init ceil height = %f\n", m->ceilHeight);
    // printf("  new ceil height = %f\n", ceilHeight);
